...
class AddMoreImages extends StatefulWidget {
  const AddMoreImages({super.key,required this.product});
...
class _AddMoreImagesState extends State<AddMoreImages> {
  ...
  late String id ;
    @override
  void initState() {
    imagesLinks = widget.product.images!.cast<String>();
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Container(
          ...
          child: Text(
            'Add images',
            style: TextStyles.style20.copyWith(color: AppColors.secondaryColor),
          )),
      content: Container(
        height: 250,
        width: 200,
  ...
  builder: (context, state) {
    return state.isLoading?const CircularProgressIndicator(): MaterialButton(
          height: 40,
          minWidth: 50,
          shape:
          ...
          onPressed: () async {
            if(imagesList.isNotEmpty){
              context.read<AddProductCubit>().updateImages(imagesLinks: imagesLinks, images: imagesList, id: id);
            }
          },
          child: Text(
            'Save',
